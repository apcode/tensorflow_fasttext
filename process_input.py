...
def ParseTextInput(textfile, labelsfie):
    examples = []
    with codecs.open(textfile, encoding="utf-8") as f1, open(labelsfile) as f2:
...
def WriteExamples(examples, outputfile, num_shards):
    ...
    for n, example in enumerate(examples):
        ...
        record["text"].bytes_list.value.extend(example["text"])
        record["label"].int64_list.value.append(example["label"])
        if "ngrams" in example:
            records["ngrams"].byes_list.value.extend(example["ngrams"])
...
def WriteVocab(examples, vocabfile):
    ...
    with codecs.open(vocabfile, encoding="utf-8", "w") as f:
...
def main(_):
    # Check flags
    if not (facebook_input or (FLAGS.text_input and FLAGS.labels)):
        print >>sys.stderr, \
